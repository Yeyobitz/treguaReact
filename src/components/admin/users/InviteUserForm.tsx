...
export function InviteUserForm({ onClose }: InviteUserFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const invitation = await createInvitation({ role }, user.uid);
      showToast(`Código de invitación generado: ${invitation.code}`, 'success');
      onClose();
    } catch (error: any) {
      console.error('Error generating invitation:', error);
      showToast(error.message || 'Error al generar el código de invitación', 'error');
    } finally {
      setLoading(false);
    }
  };
